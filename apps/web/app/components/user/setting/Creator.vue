    ...
    <template v-if="status">
      <div class="space-y-2">
        ...
        <div class="flex items-center justify-between text-sm">
          ...
          <span
            :class="
              status.eligibility.galgames_published >= status.eligibility.need_galgames
                ? 'text-success'
                : 'text-default-500'
            "
          >
        ...
        <div class="flex items-center justify-between text-sm">
          ...
          <span
            :class="
              status.eligibility.reviews_100 >= status.eligibility.need_reviews
                ? 'text-success'
                : 'text-default-500'
            "
          >
      ...
      <div v-if="status.application" class="flex items-center gap-2 text-sm">
        <span>当前申请：</span>
        <KunChip :color="STATUS_COLOR[status.application.status] || 'default'">
      ...
      <template v-if="status.application?.status !== 'pending'">
        ...
        <div class="flex justify-end">
          <KunButton :disabled="!canApply" :loading="submitting" @click="handleApply">
