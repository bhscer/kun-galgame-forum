...
| sexual/violence 取值 UI | ⚠️ **v1 仅提供 0(未评定)/1(低)/2(中)/3(高)** | wiki schema 是 `int16`，未定细则；UI 留 KunSelect 占位 + 文档注明"当前不影响展示门控,产品后续可启用" |
...
| 引入 `microdiff` 依赖 | ⚠️ **优先自实现 `arrayDiffByKey`** | 仅 ~40 行，避免新依赖；如发现复杂度爆炸再切 microdiff |
...
GET    /galgame-{tag|official|engine|series}/:id/revisions       # 🆕 列表
GET    /galgame-{tag|official|engine|series}/:id/revisions/:rev  # 🆕 单条快照
POST   /galgame-{tag|official|engine|series}/:id/revert          # 🆕 回滚 {revision: N}
```
...
| **撤销删除 UI** | ⚠️ **v1 仅做"恢复实体"基础版** | wiki 已落 `affected_galgame_ids`; 但"批量恢复关联"UI 是大特性,v1 仅提供"revert deleted revision → 实体复活,关联手动恢复"提示,不做勾选回填 UI |
...
### U1
- 🆕 kungal mapper test: wiki sample with `release_date+tba` → DTO 正确填两字段
- 🆕 前端 schema test: `release_date` 接受 `"" | "YYYY-MM-DD"`; 其他形态拒绝
- 🆕 显示组件 test: TBA=true 显示"未定"；`release_date` 缺失显示"未公布"

### U2
- 🆕 `rewriteBanners` test: covers 项注入 `cdn_url`; screenshots 项注入 `cdn_url`; `effective_banner_hash` 同时填 `effective_banner_url`
- 🆕 `getEffectiveBanner` test: 三级 fallback
- 🆕 编辑表单 test: covers 编辑 → Footer payload 包含 covers 完整数组; 不编辑 → 仍发送完整原数组(presence 约定)
- 🆕 SnapshotDiff test: covers 对象数组 diff 显示 `+added/−removed/~changed{field}`

### U3
- 🆕 `ToWikiPath` test: `/galgame-tag/:id/revisions` 等 4 实体 × 3 端点全部映射正确
- 🆕 RevisionList 组件 test: 列表渲染 + 懒加载单条 + 回滚权限门控

### 横切
- 🆕 Footer test: `direct` 分支 PUT + reconcileAliasesLinks; PR 分支 POST; covers/screenshots 都在两条路径上正确传递
- 🆕 Rewrite 水合 test: 完整水合所有字段含 covers/screenshots
