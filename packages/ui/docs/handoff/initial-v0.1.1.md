# v0.0.1 → v0.1.1 handoff — 初始大迁移

> 给 kungal / moyu 的 KunUI 同步文档。本文是 **纯增量**：列出你需要做什么，不讲历史。
> 设计动机、复审过程见 [`../architecture/design-principles.md`](../architecture/design-principles.md) 和 [`../changelog/`](../changelog/) 各版本文件。
...
完整 spec 见 [`../components/kun-tag-input.md`](../components/kun-tag-input.md)（粘贴 4 种分隔符自动拆 / IME composition 屏蔽 Enter / 长按 Backspace 不会删光 / chip 键盘 ← Backspace 删 / `tag` slot 自定义 chip）。
...
- KunUI 新 bug：同上
- 设计意图不清楚：先读 [`../README.md`](../README.md) 导航找对应章节（[`../architecture/`](../architecture/) / [`../components/`](../components/) / [`../changelog/`](../changelog/)），还不清楚再问
